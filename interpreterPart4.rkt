...
; Functionto evaluate the class closure
(define evaluateClassClosure
  (lambda (stmt state return break continue throw)
    (putInState (operand1 stmt) (list (getParentName operand2) ))))
...
; Function to get the list of instance variables and functions given the body.
(define instanceVariableAndFunctionList
  (lambda (stmt megaList) ;megaList is (list funcNames funcClosures varNames varClosures)
    (cond
      ((null? stmt) megaList)
      ((list? (car stmt)) (instanceVariableAndFunctionList (cdr stmt) (instanceVariableAndFunctionList (car stmt) megaList))
      ((eq? 'function (car stmt)) (evaluateFunctionClosure stmt state return break continue throw))
